...
<head>
    ...
    <title><?= $title ?? lang('partners') ?></title>
    ...
    <style>
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}html{scroll-behavior:smooth}body{font-family:var(--font);color:var(--text);background:#fff;line-height:1.7}a{text-decoration:none;color:inherit;transition:var(--transition)}img{max-width:100%;height:auto;display:block}ul{list-style:none}.container{max-width:1200px;margin:0 auto;padding:0 1.5rem}
        ...
        .partners-section{padding:5rem 1.5rem}.partners-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:2rem;justify-items:center}.partner-logo{width:180px;height:100px;background:#fff;border-radius:var(--radius-lg);border:1px solid var(--border-light);display:flex;align-items:center;justify-content:center;padding:1.25rem;transition:var(--transition)}.partner-logo:hover{transform:translateY(-5px);box-shadow:var(--shadow-md);border-color:var(--primary)}.partner-logo img{max-width:100%;max-height:100%;object-fit:contain;filter:grayscale(100%);opacity:.6;transition:var(--transition)}.partner-logo:hover img{filter:grayscale(0);opacity:1}
...
<div class="page-hero"><h1><?=lang('partners')?></h1><p><?=lang('partners_subtitle')?></p><div class="breadcrumb"><a href="<?=url($siteBase)?>"><?=lang('home')?></a><span>/</span><span class="current"><?=lang('partners')?></span></div></div>
<div class="partners-section"><div class="container">
    <?php if(!empty($partnerItems)):?>
    <div class="partners-grid"><?php foreach($partnerItems as$p):?>
        <div class="partner-logo">
            <?php if(!empty($p->logo)):?>
                <?php if(!empty($p->link)):?><a href="<?=htmlspecialchars($p->link)?>" target="_blank" rel="noopener"><?php endif;?>
                <img src="<?=upload($p->logo)?>" alt="<?=htmlspecialchars(localized($p, 'name') ?: $p->name ?? '')?>">
                <?php if(!empty($p->link)):?></a><?php endif;?>
            <?php else:?><span style="font-weight:700;color:var(--text);"><?=htmlspecialchars(localized($p, 'name') ?: $p->name ?? '')?></span><?php endif;?>
        </div>
    <?php endforeach;?></div>
    <?php else:?><div style="text-align:center;padding:4rem 0;color:var(--text-secondary);"><i class="fas fa-handshake" style="font-size:3rem;margin-bottom:1rem;color:var(--primary-50);"></i><p><?=lang('no_partners')?></p></div><?php endif;?>
</div></div>
