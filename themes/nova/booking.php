...
<head>
    ...
    <title><?= $title ?? lang('booking') ?></title>
    ...
    <style>
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}html{scroll-behavior:smooth}body{font-family:var(--font);color:var(--text);background:#fff;line-height:1.7}a{text-decoration:none;color:inherit;transition:var(--transition)}img{max-width:100%;height:auto;display:block}ul{list-style:none}.container{max-width:1200px;margin:0 auto;padding:0 1.5rem}
        ...
        .booking-section{padding:5rem 1.5rem}.booking-card{max-width:700px;margin:0 auto;background:#fff;border-radius:var(--radius-lg);padding:2.5rem;box-shadow:var(--shadow);border:1px solid var(--border-light)}.booking-card h2{text-align:center;font-size:1.4rem;font-weight:700;margin-bottom:2rem}.form-group{margin-bottom:1.25rem}.form-group label{display:block;font-weight:600;font-size:.88rem;margin-bottom:.4rem}.form-group input,.form-group textarea,.form-group select{width:100%;padding:.8rem 1rem;border:1.5px solid var(--border-light);border-radius:8px;font-family:var(--font);font-size:.92rem;color:var(--text);transition:var(--transition);background:var(--bg-alt)}.form-group input:focus,.form-group textarea:focus,.form-group select:focus{outline:none;border-color:var(--primary);box-shadow:0 0 0 3px var(--primary-50)}.form-group textarea{resize:vertical;min-height:100px}.btn-submit{display:inline-flex;align-items:center;gap:.5rem;padding:.85rem 2rem;background:var(--gradient);color:#fff;border:none;border-radius:var(--radius);font-weight:700;font-size:.95rem;font-family:var(--font);cursor:pointer;transition:var(--transition);box-shadow:var(--shadow-primary);width:100%;justify-content:center}.btn-submit:hover{transform:translateY(-2px)}
    ...
    @media (max-width: 576px) { .booking-card .form-row { grid-template-columns: 1fr !important; } .booking-card { padding: 1.5rem; margin: 0 1rem; } }
...
<div class="page-hero"><h1><?=lang('booking')?></h1><p><?=lang('booking_subtitle')?></p><div class="breadcrumb"><a href="<?=url($siteBase)?>"><?=lang('home')?></a><span>/</span><span class="current"><?=lang('booking')?></span></div></div>
<div class="booking-section"><div class="container">
    <div class="booking-card">
        <h2><i class="fas fa-calendar-check" style="color:var(--primary);margin-inline-end:.5rem;"></i> <?=lang('book_appointment')?></h2>
        <form id="bookingForm">
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;" class="form-row">
                <div class="form-group"><label><?=lang('name')?></label><input type="text" name="name" required placeholder="<?=lang('your_name')?>"></div>
                <div class="form-group"><label><?=lang('phone')?></label><input type="tel" name="phone" required placeholder="<?=lang('your_phone')?>"></div>
            </div>
            <div class="form-group"><label><?=lang('email')?></label><input type="email" name="email" required placeholder="<?=lang('your_email')?>"></div>
            <div class="form-group"><label><?=lang('service')?></label>
                <select name="service">
                    <option value=""><?=lang('select_service')?></option>
                    <?php if(!empty($services)):foreach($services as$svc):?>
                    <option value="<?=htmlspecialchars(localized($svc, 'title') ?: $svc->title ?? '')?>"><?=htmlspecialchars(localized($svc, 'title') ?: $svc->title ?? '')?></option>
                    <?php endforeach;endif;?>
                </select>
            </div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;" class="form-row">
                <div class="form-group"><label><?=lang('date')?></label><input type="date" name="date" required></div>
                <div class="form-group"><label><?=lang('time')?></label><input type="time" name="time" required></div>
            </div>
            <div class="form-group"><label><?=lang('notes')?></label><textarea name="notes" placeholder="<?=lang('additional_notes')?>" rows="3"></textarea></div>
            <button type="submit" class="btn-submit"><i class="fas fa-calendar-check"></i> <?=lang('book_now')?></button>
        </form>
    </div>
</div></div>
<footer class="nova-footer"><div class="container"><div class="footer-grid">
    ...
    <div class="footer-col"><h4><?=lang('more_pages')?></h4><ul><li><a href="<?=url($siteBase.'/gallery')?>"><?=lang('gallery')?></a></li><li><a href="<?=url($siteBase.'/contact')?>"><?=lang('contact_us')?></a></li></ul></div>
    <div class="footer-col"><h4><?=lang('contact_info')?></h4><ul><li><a href="tel:<?=htmlspecialchars($tenant->contact_phone??'')?>" dir="ltr"><?=htmlspecialchars($tenant->contact_phone??'')?></a></li></ul></div>
