 ...
 * ============================================================================
 * الاستخدام: ارفع هذا الملف إلى مجلد المشروع الرئيسي (بجانب index.php)
 * ثم زره من المتصفح: http://your-site.com/migrate_theme_settings.php
 * بعد الانتهاء، احذف هذا الملف فوراً!
 * ============================================================================
 */

// منع الوصول المباشر من خارج المشروع
...
try {
    ...
    if (in_array('font_primary', $existing) && in_array('primary_font', array_column($results, 'name'))) {
    ...
    if (in_array('font_secondary', $existing) && in_array('secondary_font', array_column($results, 'name'))) {
    ...
    // font_size_base → base_font_size
    if (in_array('font_size_base', $existing) && in_array('base_font_size', array_column($results, 'name'))) {
    ...
    // animation_enabled → enable_animations
    if (in_array('animation_enabled', $existing) && in_array('enable_animations', array_column($results, 'name'))) {
        $pdo->exec("UPDATE `theme_settings` SET `enable_animations` = `animation_enabled` WHERE `animation_enabled` IS NOT NULL");
        $syncs++;
    }

    // عرض النتائج
    echo '<div class="status success">تم الاتصال بقاعدة البيانات بنجاح - <strong>takween</strong></div>';
...
?>
    ...
    <div class="footer">
        <strong>تنبيه:</strong> احذف هذا الملف (<code>migrate_theme_settings.php</code>) فور الانتهاء من الترقية!
    </div>
</div>
</body>
</html>
